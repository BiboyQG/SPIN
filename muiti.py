...
def get_links_from_init_page(scrape_result):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": f"You are an expert at summarizing {prompt_type} articles in JSON format. Now you are given the initial page of the {prompt_type} article, please extract the related links that may include information about the {prompt_type} from the page.",
            },
            {
                "role": "user",
        ...
        max_tokens=16384,
        temperature=0.0,
        response_model=RelatedLinks,
    )
    return response.related_links
